...
#  operating on these Tensors. Behind the scenes, Tensors can keep
#  track of a computational graph and gradients, but they’re also
#  useful as a generic tool for scientific computing.
# 
# Also unlike numpy, PyTorch Tensors can utilize GPUs to accelerate
#  their numeric computations. To run a PyTorch Tensor on GPU,
#  you simply need to cast it to a new datatype.
# 
# Here we use PyTorch Tensors to fit a two-layer network to random data.
#  Like the numpy example above we need to manually implement the
